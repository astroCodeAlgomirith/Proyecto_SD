...
\codigo{server/AccountHandler.java} extrae el id de la ruta
(\codigo{/api/accounts/\{id\}}), consulta el \codigo{CuentaRepository} y arma el
JSON a mano (sin gson) por ser la ruta caliente. La respuesta lleva
\codigo{id}, \codigo{propietario} y \codigo{balance}.
...
\codigo{401}. El metodo lee el header \codigo{Authorization}, exige el prefijo
\codigo{Bearer }, recorta el token y verifica la firma con \codigo{JWTUtil}.

...
\begin{nota}
En total las once suites suman \textbf{110 aserciones} verdes. El conteo final lo
...
\begin{itemize}
\item \codigo{pruebas/CuentaRepositoryTest.java} (15 aserciones): valida la
...
\codigo{saldoTotalCentavos()} no cambia. Tambien comprueba que ese metodo
\emph{recalcula} la suma real recorriendo las cuentas: tras mover un saldo fuera
de una transferencia que conserva, \codigo{saldoTotalCentavos()} detecta el
descuadre, algo que un contador cacheado no podria reportar.
...
\item \codigo{pruebas/ReplicacionMultiWriterTest.java} (4 aserciones): estresa la
replicacion con 8 hilos escribiendo miles de transferencias sobre pares
disjuntos en un repo lider, mientras un lector drena el log con
\codigo{registro().desde(enviado)} y lo reaplica en una replica. Al final exige
no solo la suma global conservada en ambos repos, sino la igualdad de saldo
\emph{por cuenta} lider == replica para todas las cuentas; sin la serializacion
del par seq+log bajo \codigo{seqLock} el lector se saltaria una secuencia aun no
insertada y la replica divergiria.

\item \codigo{pruebas/PuntoControlTest.java} (18 aserciones): valida el punto de
control en disco de la replica con seis casos. En \codigo{roundTrip()} guarda con
\codigo{guardarFinal} y comprueba que \codigo{cargar} restaura saldos, secuencia y
\codigo{totalTransferencias} (igual a la secuencia). \codigo{sinArchivo()}
confirma que \codigo{cargar} devuelve \codigo{-1} y deja la secuencia en 0 (arranque
desde el CSV) cuando no hay archivo, y \codigo{archivoCorrupto()} que devuelve \codigo{-1}
cuando los bytes son basura. \codigo{archivoTruncado()} corta un checkpoint valido por la mitad (cabecera
buena, cuerpo a medias) y exige \codigo{-1} con saldos y secuencia sin tocar: \codigo{cargar}
parsea todo antes de mutar, de modo que un truncado nunca deja un estado
mitad-CSV/mitad-checkpoint. \codigo{resumeE2E()} ejercita la reanudacion completa con
sockets reales: la replica alcanza al lider, persiste su punto de corte, se reinicia
desde su secuencia exacta y pide solo \codigo{X+1..} al lider, terminando con saldos
iguales al lider y \codigo{totalTransferencias} igual a la secuencia final (sin doble
aplicacion). \codigo{resetSiAdelantada()} simula una replica que viene de un checkpoint
mas nuevo que el estado del lider rezagado: el lider ordena \codigo{RESET}, el callback
\codigo{alResetear} recarga la base y la replica converge al lider en vez de quedar
divergente.
...
\item \codigo{pruebas/HandlersErroresTest.java} (18 aserciones): ejercita los
manejadores \emph{in-process} (construye una \codigo{Solicitud} a mano, sin
socket) para fijar los codigos de error. Cubre el cuerpo de \codigo{/transfer}
(JSON malo, campos faltantes, monto no numerico, misma cuenta, monto cero o
negativo y saldo insuficiente devuelven 400; cuenta inexistente, 404; la
transferencia valida, 200); el header \codigo{Authorization} tolerante con el
scheme \codigo{Bearer} sin distincion de mayusculas y con espacios extra
(\codigo{bearer x}, \codigo{BEARER x}, \codigo{Bearer\ \ x} dan 200, y sin scheme
o con token roto, 401); y el GET de un id numerico fuera del rango \codigo{int} o
inexistente, que devuelve 404 (no 400).
\end{itemize}

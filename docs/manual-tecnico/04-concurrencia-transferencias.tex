...
mantiene al centavo. La conversion a decimal solo ocurre al serializar la salida
JSON, mediante \codigo{saldoDecimal()}, que escala el entero por $10^{-2}$.
...
almacena las cuentas en un \codigo{ConcurrentHashMap} indexado por id. Los
contadores de progreso usan tipos atomicos: \codigo{AtomicLong secuencia} es el
numero de secuencia monotono que ordena las transacciones (base de la
replicacion y del log en GCS), y \codigo{totalTransferencias} cuenta los commits.
El campo \codigo{ultimaTxId} es \codigo{volatile} para publicar el ultimo seq sin
candado adicional.
...
sus centavos; se usa en pruebas para verificar la conservacion del dinero.

\begin{lstlisting}[language=Java,caption={Verificacion del invariante en CuentaRepository.java}]
...
circular. Bajo ambos candados valida el saldo, mueve el monto, asigna un seq,
registra la \codigo{Transaccion} y avisa al observador; el bloque \codigo{finally}

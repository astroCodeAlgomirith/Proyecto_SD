...
a un \codigo{Manejador} y a la marca \codigo{INLINE} o \codigo{WORKER}, y al
registrar mantiene la tabla ordenada por prefijo mas largo primero. Asi
\codigo{resolver()} solo barre las entradas en ese orden y devuelve el
\textbf{primer} prefijo que sea prefijo del path, que por construccion es el mas
largo; ese corte temprano abarata el camino caliente. La validacion del verbo
HTTP (responder 405) queda en cada handler; aqui solo se rutea por path.

\begin{lstlisting}[language=Java,caption={Orden en registrar() y primer match en resolver()}]

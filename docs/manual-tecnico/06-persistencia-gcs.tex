...
\begin{nota}
El sistema tiene dos recuperaciones con alcances distintos. El log de Cloud
Storage descrito en este capitulo cubre la \emph{caida total} de los tres nodos:
cuando se pierde el estado en memoria de todos, el lider reconstruye el estado
completo reaplicando el log desde GCS. El \emph{punto de control local} de cada
replica (\codigo{replicacion/PuntoControl.java}, configurado con
\codigo{BANCO\_CHECKPOINT}; detallado en el capitulo de replicacion) cubre el
caso opuesto: una replica que se reinicia mientras el lider sigue vivo reanuda
desde su ultima secuencia sin redescargar todo. Mecanismos complementarios, no
sustitutos.
\end{nota}
...
lista y luego descarga cada objeto, reintentando unas pocas veces ante fallos
transitorios y propagando el error si los agota (una descarga perdida corromperia
el log al reaplicarse a medias). Parsea a mano los campos simples
...
\begin{lstlisting}[language=Java,caption={Reaplicacion ordenada del log; falla ruidosa si no se puede leer}]
...
transacciones recuperadas, engancha el observador para los nuevos commits y, por
ultimo, registra un \emph{shutdown hook} que drena la cola en un apagado ordenado
(SIGTERM), pues una tx ya ackeada con HTTP 200 puede seguir pendiente de subir y
se perderia del log si el JVM termina sin vaciar la cola. La funcion
\codigo{aplicarReplica} reaplica sin revalidar saldos ni reescribir en GCS, de
modo que la recuperacion no genera un ciclo de subidas.

\begin{lstlisting}[language=Java,caption={Recuperacion antes de servir, observador y shutdown hook (MainServer)}]
